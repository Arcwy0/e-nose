...
prebuilt wheel** (`enose-3.1.2-py3-none-any.whl`, pure Python, works on any OS),
...
- The file **`enose-3.1.2-py3-none-any.whl`** (ask the maintainer / grab it from
...
pip install ".\enose-3.1.2-py3-none-any.whl[client]"
```
...
| Symptom | Fix |
|---|---|
| `pip` tries to build `scikit-learn`/`numpy` from source | You installed without the `[client]` extra (or an old wheel). Use exactly `pip install ".\enose-3.1.2-py3-none-any.whl[client]"`. |
...
pip install --no-deps ".\enose-3.1.2-py3-none-any.whl"
```
